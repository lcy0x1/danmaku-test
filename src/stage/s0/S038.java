...
public class S038 extends SpellCard implements Emiter.Ticker {
	...
	private static final int r0 = 400, r1 = 400, m = 3;
	...
	public void tick(Emiter e, int it, int ex) {
		if (e.id == 0) {
			...
			for (int i = 0; i < n0; i++) {
				double a1 = a0 + p2 / n0 * i;
				P p0 = P.polar(r0, a1).plus(pos);
				add(new DotBullet(new Dot(p0, -r0, a1, v0, wa, d0)), ex);
			}
		}
		if (e.id == 1) {
			double a0 = -it * f1 * w1;
			...
			for (int i = 0; i < n1; i++) {
				...
				P p0 = P.polar(r1, a1).plus(pos);
				add(new DotBullet(new Dot(p0, -r1, a1, v0, wa, d1)), ex);
			}
		}
		if (e.id == 2) {
			double a0 = pos.atan2(getPlayer().pos);
			for (int i = 0; i < n2; i++) {
				double a1 = a0 + p2 / n2 * i;
				for (int j = 0; j < m; j++)
					add(new DotBullet(new Dot(pos.copy(), P.polar(v1 + dv * j, a1), d2)), ex);
			}
			for (int i = 0; i < n2; i++) {
